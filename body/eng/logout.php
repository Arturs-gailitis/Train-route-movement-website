        ...
        <div id="opcijuLaukums">
            ...
            <div class="fonaIzmaiņas" id="valodaIzmaiņas">
                ...
                <select name="valoda" id="valoda">
                    <option value="http://localhost:8000/body/eng/logout.php">English</option>
                    <option value="http://localhost:8000/body/lv/iziet.php">Latvian</option>
                </select>
            </div>

        </div>
    </div>
    <div id="iziesanasLaukums">
        <?php if (isset($_SESSION['lietotajvards'])): ?>
            <form method="POST">
                <h2>Are you sure you want to leave your profile?</h2>
                <button type="submit" class="btn btn-primary" id="iziesanasPoga">Get out</button>
            </form>
        <?php else: ?>
            <h2>You have logged out of your profile.</h2>
            <button id="ietUzSakumu" class="btn btn-primary">Go to homepage</button>
        <?php endif ?>
    </div>
</body>
